 ...
 */
// FIXME Use the one from atbash-utils
public class ClassUtils {
    ...
    // FIXME, This is updated from the Octopus version one !! Don't forget to copy back into atbash-utils.
    public static Object newInstance(Class clazz, Object... args) {
        ...
        // Because JDK version Class.getConstructor(argTypes); matches only exact types.
        Constructor ctor = matchConstructor(clazz, argTypes);
    ...
    private static Constructor matchConstructor(Class clazz, Class[] argTypes) {
        ...
        for (Constructor constructor : clazz.getConstructors()) {
            if (constructor.getParameterTypes().length == argTypes.length) {
                if (matchParameterTypes(constructor.getParameterTypes(), argTypes)) {
                    constructors.add(constructor);
                }
            }
        }
        if (constructors.size() != 1) {
            throw new InstantiationException("No constructor found matching the parameters"); // TODO Give parameter types in message.
        }
        return constructors.get(0);
    }

    private static boolean matchParameterTypes(Class[] parameterTypes, Class[] argTypes) {
        boolean result = true;
